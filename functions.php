...
function Vz_create_shortcode_portfolio_post_type(){
  
    $args = array(
                    'post_type'      => 'portfolio', // don't forget to replace it with your custom post type name
                    'posts_per_page' => '5',
    ...
    if($query->have_posts()) :
        ...
        $result  = $result . '<div class="portfolio-poster">' . get_the_post_thumbnail() . '</div>';
        $result = $result . '<div class="portfolio-name">' . get_the_title() . '</div>';
        $result = $result . '<div class="portfolio-desc">' . get_the_content() . '</div>'; 
        $result = $result . '</div>';
...
add_shortcode( 'portfolio-list', 'Vz_create_shortcode_portfolio_post_type' ); 
